...
if($USER_TYPE=="STUDENT") $pagina="payment.htm";  //valor por defecto
else $pagina="tutor-subjects.html";  //valor por defecto
  ...
  else
  {
     ...
     else
     {
         ...
         if($fila) //user exists in DB
	     {
			...
			{	
				$query="UPDATE USERS SET NAME='$form_name',EMAIL='$form_email',PASSWORD=HEX(AES_ENCRYPT('$form_password', UNHEX(SHA2('babel',512)))),GENDER=$form_gender,SCHOOLTYPE=$form_school WHERE USERCODE='$USERCODE'";
				$resultado=genesis_exec($con,$query);
                $mensaje=1;
				$_SESSION['NOMBRE_USUARIO']=$form_name;	
	     ...
	     {               
			...
			{	
				session_start();   	
				$SESION=session_id();
				$query="INSERT INTO USERS (NAME,SIGNUPDATE,EMAIL,PASSWORD,BALANCE,SESSIONID,USERTYPE,GENDER,SCHOOLTYPE) VALUES ('$form_name',NOW(),'$form_email',HEX(AES_ENCRYPT('$form_password', UNHEX(SHA2('babel',512)))),0,'$SESION','$USER_TYPE',$form_gender,$form_school)";
				$resultado=genesis_exec($con,$query);
				...
				if($USERCODE!='')
				{
					...
					$_SESSION['TIPO_USUARIO']=$USER_TYPE;   	//usar esto si register_globals=off
					$_SESSION['COD_USUARIO']=$USERCODE;   	//this will set hay_session to true  
					$_SESSION['BALANCE']='0';
					$_SESSION['ACTIVE_LESSONSTATUS']='0';
					$_SESSION['LESSON_CODEVAR']="";
					$USER_NAME=$form_name;
